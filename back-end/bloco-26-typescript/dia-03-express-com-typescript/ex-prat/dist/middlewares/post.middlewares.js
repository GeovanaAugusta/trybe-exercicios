...
function validationPost(req, res, next) {
    ...
    if (post.author.length < 3)
        return res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).send('O nome do autor deve ter no mínimo 3 caracteres');
    if (post.category.length < 3)
        return res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).send('O nome da categoria deve ter no mínimo 3 caracteres');
...
exports.default = validationPost;
// Atividade 2 🚀
// Objetivos:
