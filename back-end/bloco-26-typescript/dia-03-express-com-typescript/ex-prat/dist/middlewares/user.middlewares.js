...
function validationUser(req, res, next) {
    ...
    if (!reg.test(user.email)) {
        return res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).send('E-mail inválido');
    }
    if (user.name.length < 3)
        return res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).send('O nome deve ter no mínimo 3 caracteres');
    if (user.password.length < 6 || user.password.length > 12)
        return res.status(http_status_codes_1.StatusCodes.BAD_REQUEST).send('A senha deve ter no mínimo 6 e no máximo 12 caracteres');
    next();
}
